...
export default function Home() {
  ...
  return (
    <main className="bg-black text-white min-h-screen flex flex-col items-center px-6 font-mono relative overflow-hidden">
      ...
      {/* 2. HERO */}
      <section className="text-center max-w-4xl z-10 py-10">
      ...
      <section className="max-w-2xl w-full mt-24 bg-zinc-950/80 border border-red-900/30 p-8 relative z-10 backdrop-blur-sm">
        ...
        <p className="text-zinc-300 leading-relaxed min-h-15 text-sm md:text-base italic">
          {text}
      ...
      {/* 4. ANALYSIS */}
      <section className="max-w-5xl w-full mt-40 px-4 z-10">
      ...
      <section className="max-w-6xl w-full mt-48 flex flex-col lg:flex-row items-center gap-16 px-4 z-10">
        <div className="relative w-full lg:w-1/2 aspect-square group">
          <div className="absolute inset-0 bg-red-600 opacity-10 blur-3xl group-hover:opacity-20 transition-opacity"></div>
          <Image 
            src="/Paradox_Shift.jpeg" 
            alt="The Paradox Shift E-book Cover"
            fill
            className="object-cover border border-zinc-800 grayscale hover:grayscale-0 transition-all duration-700 shadow-2xl"
          />
          <div className="absolute bottom-4 left-4 bg-black/80 p-4 border-l-2 border-red-600 backdrop-blur-md">
            <span className="text-[10px] text-red-600 block font-bold mb-1 tracking-widest uppercase">File_Type: Source_Code.pdf</span>
            <span className="text-[10px] text-zinc-400 block font-bold uppercase tracking-widest italic">Includes: RealityOS_Alpha_Key</span>
          </div>
        </div>
        <div className="w-full lg:w-1/2">
          ...
          <p className="text-red-600 mb-8 italic text-lg tracking-tight">"De handleiding voor wie de simulatie wil verlaten."</p>
          <ul className="space-y-6 text-zinc-400 font-mono text-sm">
            {[
              "De-installeer de 12 'Social Malware' scripts.",
              "Reality Scanning: Leer de patronen van het systeem zien.",
              "Timeline Jumping: Forceer beslissingen vanuit je doel (6ha).",
              "Access Key: Activeer je RealityOS Dashboard login."
            ].map((point, i) => (
              <li key={i} className="flex items-center gap-4 group">
                <span className="w-2 h-2 bg-red-600 rotate-45 group-hover:scale-150 transition-transform"></span>
                <span className="group-hover:text-white transition-colors uppercase tracking-tight">{point}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {/* 6. MANIFESTO */}
      <section className="max-w-4xl text-center mt-48 px-4 z-10 py-20 border-y border-zinc-900">
      ...
      <section className="max-w-5xl w-full mb-48 px-6 py-20 border border-zinc-900 bg-zinc-950/30 text-center relative overflow-hidden z-10 mx-auto">
        ...
        <h3 className="text-red-600 text-[10px] tracking-[0.4em] mb-4 uppercase font-bold">Hardware_Upgrade_Detected</h3>
        ...
        <p className="text-zinc-500 text-base max-w-2xl mx-auto mb-12 font-mono leading-relaxed">
      ...
      {/* 8. FOOTER */}
      <footer className="w-full max-w-7xl py-16 border-t border-zinc-900 flex flex-col items-center gap-10 text-[10px] text-zinc-600 tracking-widest uppercase z-10">
        <div className="flex flex-wrap justify-center gap-12 text-zinc-400">
          <a href="https://tiktok.com/@thearchitecthacker" target="_blank" rel="noopener noreferrer" className="hover:text-red-600 transition-colors flex items-center gap-2">
            <span className="w-1 h-1 bg-zinc-800 rounded-full"></span> [ TIKTOK ]
          </a>
          <a href="https://instagram.com/thearchitecthacker" target="_blank" rel="noopener noreferrer" className="hover:text-red-600 transition-colors flex items-center gap-2">
            <span className="w-1 h-1 bg-zinc-800 rounded-full"></span> [ INSTAGRAM ]
          </a>
          <a href="https://x.com/TheArchyHacker" target="_blank" rel="noopener noreferrer" className="hover:text-red-600 transition-colors flex items-center gap-2">
            <span className="w-1 h-1 bg-zinc-800 rounded-full"></span> [ X_STATION ]
          </a>
        </div>
